...
learningRateHL = 0.02;
epochs = 1000;
hiddenSizes_history{1} = hiddenSizes;
bestParams = struct();
bestJv = Inf;
i = 1;
c = tic();
...
while a < 30*60
    ...
    i = i + 1;
    i-1
    [out, weight, Jt, Jv, epoch] = train_ann(inputTrain, outTrain, hiddenSizes, learningRateOL,learningRateHL, epochs, inputValid, outValid);
    ...
    if finalJv < bestJv
        ...
        bestParams.learningRateOL = learningRateOL;
        bestParams.learningRateHL = learningRateHL;
    ...
    while t
        hiddenSizes = repmat(randi([5, 100]), 1, randi([1, 10]));
        
        for j = 1:numel(hiddenSizes_history)
            if numel(hiddenSizes_history{j}) == numel(hiddenSizes)
                if hiddenSizes_history{j} == hiddenSizes
                    t = 0;
                    break;
                end
    ...
    hiddenSizes_history{i} = hiddenSizes;
    learningRateOL = 0.01 + 0.1*rand();
    learningRateHL = 0.01 + 0.1*rand();
    epochs = 500 + randi(500);
    a = toc(c);
end

% Addestramento finale con i parametri migliori
[out,weight,Jt,Jv,epoch] = train_ann(inputTrain,outTrain,bestParams.hiddenSizes,bestParams.learningRateOL,bestParams.learningRateHL,bestParams.epochs,inputValid, outValid);
% [out, weight, Jt, Jv, epoch] = train_ann(inputTrain, outTrain, bestParams.hiddenSizes, , bestParams.epochs, inputValid, outValid);
...
% hold on 
% plot(out)
[out,weight,Jt,Jv,epoch] = train_ann(inputTrain,outTrain,[30 30 30],0.0127,0.0384,100000,inputValid,outValid);
...
function weights = updateWeights(input, hiddenOutputs, output, target, weights, learningRateOL,learningRateHL)
    ...
    for i = numHiddenLayers:-1:1
        ...
        if i == 1
            delta = (outputDelta * weights{i+1}') .* (1 - tanh(sum(input*weights{i},1)).^2);
            weights{i} = weights{i} - learningRateHL * (input' * delta);
        else
            % Calcolo del delta per lo strato nascosto corrente
            delta = (outputDelta * weights{i+1}') .* (1 - tanh(sum(hiddenOutputs{i-1}*weights{i},1)).^2);
